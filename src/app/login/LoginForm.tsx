...
import EyeSolidIcon from '../(private)/components/svgs/EyeSolidIcon';
import EyeSlashSolidIcon from '../(private)/components/svgs/EyeSlashIcon';
...
export default function LoginPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'email') {
      ...
      if (formErrors.email && validateForm({ ...formData, email: value.toLowerCase() }).email === null) {
        setFormErrors(prev => ({
          ...prev,
          email: null,
        }));
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (newErrors.email !== null && name === 'email') {
      setFormErrors(prev => ({
        ...prev,
        email: newErrors.email,
      }));
    } else if (newErrors.password !== null && name === 'password') {
      setFormErrors(prev => ({
        ...prev,
    ...
    <div className="max-w-[500px] h-[400px] w-full space-y-8 p-8 bg-gradient-to-br from-[#F0FDFC] via-white to-[#E0F5F3] rounded-lg shadow-xl flex justify-center border border-[#DFDFDF]">
      <div className="w-[280px] flex items-center justify-center flex-col">
        <div className="text-left w-full flex justify-between">
          ...
          <div className="w-[44px] h-[44px] rounded-full bg-[#B6E8E4]"></div>
        </div>
